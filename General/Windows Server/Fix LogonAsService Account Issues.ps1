...
# If this does not work, try the below, reboot and try the above again

#First prepare the configuration file in cmd -> secedit /export /cfg C:\secpol.cfg

# Open the secpol.cfg file in a text editor and add / append the following line under the [Privilege Rights] section to ensure "NT SERVICE\ALL SERVICES" has the "SeServiceLogonRight" permission: SeServiceLogonRight = *S-1-5-80-0

# run the following command in cmd: secedit /configure /db secedit.sdb /cfg C:\secpol.cfg /overwrite